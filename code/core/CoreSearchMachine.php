...
class SearchEngineCoreSearchMachine extends SS_Object {
	 ...
	 * @var string
	 */
	private static $class_name_for_search_provision = "SearchEngineProvider_MYSQLFullText";
	...
	public function run($searchPhrase, $filterProviders = array(), $sortProvider = ""){
		...
		if(!$listOfIDsRAW) {
			if($this->debug) {$filterExecutedRAW = true; }
			//run search to search engine specific engine
			$searchProviderName = Config::inst()->get("SearchEngineCoreSearchMachine", "class_name_for_search_provision");
		...
		if(!$listOfIDsSQL) {
			if($this->debug) {$filterExecutedSQL = true;}
			if(is_array($filterProviders) && count($filterProviders)) {
				...
				foreach($filterClassesWithValues as $filterClassName => $filterValues) {
					...
					if($filter = $filterObjects[$filterClassName]->getSqlFilterArray($filterValues, $this->debug)) {
						...
						if($this->debug) {$filterStringForDebug .= print_r($filter, 1);}
					}
				}
			}
		...
		if($hasCustomSort) {
			if($this->debug) {$startTimeForCustomSort = microtime(true);}
			$dataList = $sortProviderObject->doCustomSort($dataList, $searchRecord, $this->debug);
			if($this->debug) {$endTimeForCustomSort = microtime(true);}
		}
